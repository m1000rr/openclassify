...
class Listing extends Model implements HasMedia
{
    ...
    public function relatedSuggestions(int $limit = 8): Collection
    {
        $baseQuery = static::query()
            ->publicFeed()
            ->with(['category:id,name', 'videos', 'media'])
    ...
    public static function sellerPanelStatusOptionsFor(?string $currentStatus = null): array
    {
        $options = [
            'pending' => 'Pending',
            'sold' => 'Sold',
    ...
    public function isPubliclyExpired(): bool
    {
    ...
    {
        return $this->statusValue() === 'active' && ! $this->isPubliclyExpired();
    }
    ...
    {
        if ($this->statusValue() === 'sold') {
            return;
        }

        $this->status->transitionTo(SoldListingStatus::class);
        $this->save();
    }

    public function republish(): void
    {
        abort_unless($this->statusValue() === 'expired', 403);

        $this->status->transitionTo(ActiveListingStatus::class);
        $this->forceFill([
    ...
    public function updateFromPanel(array $attributes): void
    {
        ...
        if (array_key_exists('status', $attributes)) {
            $requestedStatus = (string) $attributes['status'];

            if ($requestedStatus === 'active' && $this->statusValue() !== 'active') {
                $requestedStatus = $this->statusValue();
            }

            if ($requestedStatus !== $this->statusValue()) {
                $this->transitionPanelStatusTo($requestedStatus);
            }
        }
