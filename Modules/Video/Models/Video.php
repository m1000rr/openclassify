...
class Video extends Model
{
    ...
    public static function createFromTemporaryUpload(Listing $listing, TemporaryUploadedFile $file, array $attributes = []): self
    {
        ...
        $path = $file->storeAs(
            DemoStoragePath::prefix(trim((string) config('video.upload_directory', 'videos/uploads').'/'.$listing->getKey(), '/')),
    ...
    {
        return DemoStoragePath::prefix(trim(
    ...
    protected function scheduleProcessingIfNeeded(): void
    {
        if (
            ...
            || ((! $this->wasRecentlyCreated) && (! $this->wasChanged('upload_path')))
        ) {
            return;
        }

        $dispatch = ProcessVideo::dispatch(
            $this->getKey(),
            config('demo.active_schema'),
            config('demo.active_uuid'),
        )->afterCommit();

        if (filled(config('demo.active_uuid'))) {
            $dispatch->onConnection('sync');
        } else {
            $dispatch->onQueue((string) config('video.queue', 'videos'));
        }
    }
