        ...
        <form method="post" action="{{ route('profile.destroy') }}" class="space-y-6 p-6 sm:p-8">
            ...
            <div>
                ...
                <p class="mt-2 text-sm leading-6 text-slate-500">
                    @if(auth()->user()?->hasLinkedSocialAccount())
                        {{ __('Type DELETE to confirm permanent deletion. This action cannot be reversed.') }}
                    @else
            ...
            <div class="account-field">
                @if(auth()->user()?->hasLinkedSocialAccount())
                    <label for="confirmation" class="account-label">{{ __('Confirmation') }}</label>
                    <input
                        id="confirmation"
                        name="confirmation"
                        type="text"
                    ...
                    @foreach ($errors->userDeletion->get('confirmation') as $message)
