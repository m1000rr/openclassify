...
class ConversationController extends Controller
{
    ...
    public function start(Request $request, Listing $listing): RedirectResponse | JsonResponse
    {
        ...
        if (! $listing->isPubliclyVisible()) {
            if ($request->expectsJson()) {
                return response()->json(['message' => 'This listing is not available for messaging.'], 422);
            }

            return back()->with('error', 'This listing is not available for messaging.');
        }
    ...
    private function resolveMessageFilter(Request $request): string
    {
        ...
        return in_array($messageFilter, ['all', 'unread'], true) ? $messageFilter : 'all';
    }
